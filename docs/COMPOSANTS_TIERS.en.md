...
Pandoc has been removed from the new Windows build, so its GPL terms no longer govern distribution of that package. Before public distribution, compliance of the **remaining** components, especially Tesseract's native dependencies, must be checked and described in the Release notes.
